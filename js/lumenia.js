...
(function () {
  ...
  function toggleDark() {
    ...
    document.querySelectorAll('.theme-toggle').forEach(applyDarkToggleIcon);
  }

  function injectDarkToggle() {
    /* ── Bouton dans le header (desktop) ── */
    if (siteHeader && !document.getElementById('dark-toggle')) {
      ...
      /* Insérer juste avant le hamburger (qu'il soit direct child ou dans la nav) */
    ...
    } else if (document.getElementById('dark-toggle')) {
      var existing = document.getElementById('dark-toggle');
      applyDarkToggleIcon(existing);
      existing.addEventListener('click', toggleDark);
    }

    /* ── Bouton flottant pour mobile (toujours visible sur mobile) ── */
    if (!document.getElementById('dark-toggle-float')) {
      var floatBtn = document.createElement('button');
      floatBtn.id        = 'dark-toggle-float';
      floatBtn.className = 'theme-toggle theme-toggle-float';
      floatBtn.type      = 'button';
      applyDarkToggleIcon(floatBtn);
      floatBtn.addEventListener('click', toggleDark);
      document.body.appendChild(floatBtn);
    }

    /* ── Bouton dans le menu mobile (si menu hamburger accessible) ── */
