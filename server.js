...
const RATE_LIMIT_MAX    = 30;        // max 30 requêtes API / minute / IP
...
<script src="/js/interactions.js"></script>
  ...
  /* POST /api/contact — body: { nom, email, sujet, message } */
  if (route === '/api/contact' && method === 'POST') {
    const body    = await readBody(req);
    const nom     = (body.nom     || '').trim().slice(0, 100);
    const email   = (body.email   || '').trim().slice(0, 200);
    const sujet   = (body.sujet   || 'Autre').trim().slice(0, 100);
    const message = (body.message || '').trim().slice(0, 5000);
    if (!nom || !email || !message) return jsonResponse(res, 400, { error: 'Champs manquants' });
    const emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRe.test(email)) return jsonResponse(res, 400, { error: 'Email invalide' });
    let contacts = [];
    try { contacts = JSON.parse(fs.readFileSync(CONTACTS_FILE, 'utf8')); } catch (_) {}
    contacts.push({ nom, email, sujet, message, date: formatDate(new Date()), id: generateId() });
    fs.writeFileSync(CONTACTS_FILE, JSON.stringify(contacts, null, 2), 'utf8');
    console.log(`[contact] Nouveau message de ${nom} <${email}> — ${sujet}`);
    return jsonResponse(res, 200, { ok: true });
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url.startsWith('/api/')) {
    ...
    if (isRateLimited(ip)) {
      return jsonResponse(res, 429, { error: 'Trop de requêtes. Veuillez patienter une minute.' });
    }
  ...
  if (urlPath === '/favicon.ico') {
    res.writeHead(301, { 'Location': '/attached_assets/lumenia-icon.svg' });
    res.end();
    return;
  }
